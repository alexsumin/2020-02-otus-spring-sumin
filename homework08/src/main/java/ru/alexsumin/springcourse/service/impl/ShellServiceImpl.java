...
public class ShellServiceImpl implements ShellService {
    ...
    public String updateBook(@ShellOption({"id"}) String bookId,
                             @ShellOption({"title"}) String title,
                             @ShellOption({"published"}) String published,
                             @ShellOption({"author"}) String author,
                             @ShellOption({"genre"}) String genre) {
        return libraryService.updateBook(bookId, title, published, author, genre);
    }

    @Override
    @ShellMethod(key = "show_comments_for_book", value = "Show comments of book. Args: id\n" +
            "           Example:\n" +
            "               show_comments_for_book 1")
    public String showCommentsForBook(String bookId) {
